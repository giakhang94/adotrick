		...
		if(!$this->input->post('')){
			...
			if($id == 0){
				...
				if ( ! $this->upload->do_upload("thumb")){
					...
					if ($id_thumb){
						$new_thumb_name = strval($id_thumb)."_".strlval($thumb).$file_ext;
						...
						$this->load->model('video_model');
						$res = $this->video_model->updateThumbName($id_thumb, $new_thumb_name, $old_path, $new_path,$target_folder);
						if($res){
				...
				if ( ! $this->upload->do_upload("thumb")){
					...
					{
						$new_thumb_name = strval($id)."_".strlval($thumb).$file_ext;
						$old_path = $config['upload_path'].$thumb.$file_ext;
						$new_path = $config['upload_path'].$new_thumb_name;
						$new_target = $target_folder.$new_thumb_name;
						$this->load->model('video_model');
						$res = $this->video_model->updateThumbName($id, $new_thumb_name, $old_path, $new_path,$target_folder);
						if($res){
							header("Location: ".base_url()."videos/listVideo");
						}
						else {
